@extends('layouts.app')
@section('title', 'Edit Maintenance Window')
...
<div class="max-w-2xl">
    <div class="flex items-center gap-3 mb-5">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-800">Edit Maintenance Window</h1>
            <p class="text-xs text-sky-500">{{ $maintenance->title }}</p>
        </div>
    </div>

    <div class="bg-white rounded-2xl border border-sky-100 shadow-sm p-6">
        <form method="POST" action="{{ route('maintenance.update', $maintenance) }}" class="space-y-5">
            @csrf @method('PUT')
